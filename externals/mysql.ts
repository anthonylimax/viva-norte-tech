...
export class DatabaseMySQL implements IDatabaseAdapter {
    ...
    async GetAllAnnouncements(): Promise<any> {
        try{
            const [announcement] : any[] = await this.database.query("select * from Announcements as a join filters as f on f.id_announcement = a.id")
            const [pictures] : any[] = await this.database.query("select url, id, id_announcements from Pictures");
            const content : any[] = [];
            console.log(announcement)
            announcement.forEach((element : any) => {
                const pics = pictures.filter((x : any) => x.id_announcements == element.id_announcement)   
